...
$current_level = $user['PlayerLevel'];
$imgSrc = "";
$result_path = "";
$msg = "";
...
/***********************************************************
	...
	***********************************************************/	
	...
	//Set Upload directory    
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/safe3/assets/images/profile_pics';
	//Set File name	
	...
	***********************************************************/
	...
	//the image src
		...
		//Insert image into database
			$insert_pic_query = mysqli_query($con, "UPDATE users SET ProfilePicture='$result_path' WHERE UserName='$userLoggedIn'");
...
<?php
	if($current_level >= 2) { //the "Upload Profile Picture" functionality will only work if the current user is at Level 2 or more
	?>
	...
	<div id="formExample">
		...
		<form action="upload_profile_picture.php" method="post"  enctype="multipart/form-data">
			Upload an image to use as your profile picture.<br /><br />
	...
	if($imgSrc){ //if an image has been uploaded display cropping area?>
		...
		<div id="CroppingContainer" style="width:800px; max-height:600px; background-color:#FFF; margin-left: -200px; position:relative; overflow:hidden; border:2px #666 solid; z-index:2001; padding-bottom:0px;">  
			...
			<div id="CropImageForm" style="width:100px; height:30px; float:left; margin:10px 0px 0px 40px;" >  
				<form action="upload_profile_picture.php" method="post" onsubmit="return checkCoords();">
			...
			<div id="CropImageForm2" style="width:100px; height:30px; float:left; margin:10px 0px 0px 40px;" >  
				<form action="upload_profile_picture.php" method="post" onsubmit="return cancelCrop();">
		...
		echo '<div class="main_column column">
				<p>You do not have a high enough level to unlock this functionality.</p>
				</div>';
	}
?>
